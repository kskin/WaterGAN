...
#            # Load samples in batches of 100
            ...
            for idx in xrange(0, sample_batch_idxs):
                ...
                if self.is_crop:
                    sample_air_batch = [scipy.misc.imresize(scipy.misc.imread(sample_air_batch_file),(self.sw,self.sh,3,3)) for sample_air_batch_file in sample_air_batch_files]
...
#    feed_dict={self.z:sample_z,self.air_inputs:sample_air_images,self.depth_inputs: sample_depth_images})
               ...
               # print(sample_fake_images.shape)
                ...
                for img_idx in range(0,self.batch_size):
                    ...
                    sample_im = sample_ims[img_idx,0:self.sw,0:self.sh,0:3]
                    ...
                    sample_im = sample_air_images[img_idx,0:self.sh,0:self.sw,0:3]
                    #sample_im = conv_ims[img_idx,0:256,0:256,0:3]
                    sample_im = np.squeeze(sample_im)
                    scipy.misc.imsave(out_name,sample_im)
                    out_name = "paper/depth_%0d_%02d_%02d.mat" % (epoch, img_idx,idx)
                    sample_im = sample_depth_images[img_idx,0:self.sw,0:self.sh,0]
                    sample_im = np.squeeze(sample_im)
                ...
                #print(disc_result)
                #outfile = "my_results-lizard-c.txt"
                #fh = open(outfile,"a")
                #for img_idx in range(0,self.batch_size):
