...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    self.sample_air_inputs = tf.placeholder(
      tf.float32, [self.batch_size] + [256,256,3], name='air_images')
    ...
    self.sample_fake_inputs = tf.placeholder(
      tf.float32, [self.batch_size] + [256,256,3], name='depth')
...
#    feed_dict={self.z:sample_z,self.air_inputs:sample_air_images,self.depth_inputs: sample_depth_images})
               ...
               # print(sample_fake_images.shape)
                sample_fake_images_small = np.empty([0,256,256,3])
                for img_idx in range(0,self.batch_size):
                    out_name = "jamaica_c/fake_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                    ...
                    out_name = "jamaica_c/air_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                    sample_im = sample_air_images[img_idx,0:256,0:256,0:3]
                    #sample_im = conv_ims[img_idx,0:256,0:256,0:3]
                    sample_im = np.squeeze(sample_im)
                    scipy.misc.imsave(out_name,sample_im)
                    out_name = "jamaica_c/depth_%0d_%02d_%02d.mat" % (epoch, img_idx,idx)
                    ...
                    sample_fake = scipy.misc.imresize(sample_fake,[256,256,3],interp='bicubic')
              ...
              #  disc_result = np.squeeze(disc_result)
               ...
               # print(disc_result)
               # outfile = "my_results-jamaica-c.txt"
               # fh = open(outfile,"a")
               # for img_idx in range(0,self.batch_size):
