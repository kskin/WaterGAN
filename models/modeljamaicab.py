...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    self.sample_air_inputs = tf.placeholder(
      tf.float32, [self.batch_size] + [64,64,3], name='air_images')
    self.sample_depth_inputs = tf.placeholder(
      tf.float32, [self.batch_size] + [64,64,1], name='depth')
    self.sample_fake_inputs = tf.placeholder(
      tf.float32, [self.batch_size] + [64,64,3], name='depth')
    ...
    self.sample_z = tf.placeholder(
      tf.float32, [None, self.z_dim], name='z')
    self.G,eta_r,eta_g,eta_b,beta_r,beta_g,beta_b = self.wc_generator(self.z,air_inputs, depth_inputs)
    if self.is_stereo:
    ...
    self.beta_r_loss = -tf.minimum(tf.reduce_min(beta_r),0)*1000
    self.beta_g_loss = -tf.minimum(tf.reduce_min(beta_g),0)*1000
    self.beta_b_loss = -tf.minimum(tf.reduce_min(beta_b),0)*1000
    self.beta_r_loss1 = -tf.minimum(tf.reduce_min((1.0-beta_r)),0)*1000
    self.beta_g_loss1 = -tf.minimum(tf.reduce_min((1.0-beta_g)),0)*1000
    self.beta_b_loss1 = -tf.minimum(tf.reduce_min((1.0-beta_b)),0)*1000

#    self.beta_loss = -tf.minimum(tf.reduce_min(beta),0)
    self.g_loss = self.g_loss+ self.eta_r_loss + self.eta_g_loss +self.eta_b_loss + self.eta_r_loss1 + self.eta_g_loss1 +self.eta_b_loss1+self.beta_r_loss + self.beta_g_loss +self.beta_b_loss + self.beta_r_loss1 + self.beta_g_loss1 +self.beta_b_loss1
      ...
      #  with tf.device('/gpu:2'):
        ...
        if np.mod(counter, 5) == 1:
         ...
         # print(self.sess.run('discriminator_1/Sigmoid:0'))
          ...
          print(self.sess.run('wc_generator/g_atten/g_beta_r:0'))
          print(self.sess.run('wc_generator/g_atten/g_beta_g:0'))
          print(self.sess.run('wc_generator/g_atten/g_beta_b:0'))
...
#print(self.sess.run('wc_generator/g_vig/w:0'))
        ...
        #print(self.D_)
            ...
            for idx in xrange(0, sample_batch_idxs):
                ...
                if self.is_crop:
                    sample_air_batch = [scipy.misc.imresize(scipy.misc.imread(sample_air_batch_file),(64,64,3)) for sample_air_batch_file in sample_air_batch_files]
                    sample_water_batch = [scipy.misc.imresize(scipy.misc.imread(sample_water_batch_file),(64,64,3)) for sample_water_batch_file in sample_water_batch_files]
...
#    feed_dict={self.z:sample_z,self.air_inputs:sample_air_images,self.depth_inputs: sample_depth_images})
                ...
                sample_fake_images = sample_ims[:,0:64,0:64,0:3]
               # print("here")
               ...
               # print(sample_fake_images.shape)
                sample_fake_images_small = np.empty([0,64,64,3])
                for img_idx in range(0,self.batch_size):
                    out_name = "jamaica_b/fake_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                    sample_im = sample_ims[img_idx,0:64,0:64,0:3]
                    sample_im = np.squeeze(sample_im)
                    #print(sample_im.shape)
                    scipy.misc.imsave(out_name,sample_im)
                    out_name = "jamaica_b/air_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                    sample_im = sample_air_images[img_idx,0:64,0:64,0:3]
                    #sample_im = conv_ims[img_idx,0:256,0:256,0:3]
                    sample_im = np.squeeze(sample_im)
                    scipy.misc.imsave(out_name,sample_im)
                    out_name = "jamaica_b/depth_%0d_%02d_%02d.mat" % (epoch, img_idx,idx)
                    sample_im = sample_depth_images[img_idx,0:64,0:64,0]
                    sample_im = np.squeeze(sample_im)
                    sio.savemat(out_name,{'depth':sample_im})
                    sample_fake = sample_fake_images[img_idx,0:64,0:64,0:3]
                    sample_fake = np.squeeze(sample_fake)
                    sample_fake = scipy.misc.imresize(sample_fake,[64,64,3],interp='bicubic')
                ...
                print(disc_result)
                outfile = "my_results-jamaica-b.txt"
                fh = open(outfile,"a")
                for img_idx in range(0,self.batch_size):
  ...
  def wc_generator(self, z, image, depth, y=None):
    with tf.variable_scope("wc_generator") as scope:
      ...
      with tf.variable_scope("g_atten"):
          ...
          init_br = tf.random_normal([1,1,1],mean=0.01,stddev=0.005,dtype=tf.float32)
          beta_r = tf.get_variable("g_beta_r",initializer=init_br)
          init_bb = tf.random_normal([1,1,1],mean=0.01,stddev=0.005,dtype=tf.float32)
          beta_b = tf.get_variable("g_beta_b",initializer=init_bb)
          init_bg = tf.random_normal([1,1,1],mean=0.01,stddev=0.005,dtype=tf.float32)
  ...
  #        h9z, [64,32,32, 1], name='g_h2_deconv', with_w=True)
   ...
   #       h8z, [64,64,64, 1], name='g_h1_deconv', with_w=True)
    ...
    #  h2z = self.h2z
      ...
      #hav = tf.mul(h0,h1)
      h_out = tf.mul(h1,h0)
      return h_out, eta_r,eta_g,eta_b, beta_r,beta_g,beta_b
  ...
  def wc_sampler(self, z, image, depth, depth_small,y=None):
    ...
    #  print(self.h2z)
     ...
     # h_out = tf.mul(h2z, h0)
      ...
      h_rxl = tf.squeeze(h_r,axis=3)
      h_gxl = tf.squeeze(h_g,axis=3)
      h_bxl = tf.squeeze(h_b,axis=3)
