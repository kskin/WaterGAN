...
#    feed_dict={self.z:sample_z,self.air_inputs:sample_air_images,self.depth_inputs: sample_depth_images})
                ...
                for img_idx in range(0,self.batch_size):
                    out_name = "sample_outlizardavc/fake_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
...
#+tf.mul(B,(1.0-eta_d))
    ...
    #  print(self.h2z)
     ...
     # h_out = tf.mul(h2z, h0)
      ...
      h2z = self.h2z
      h_rxl = tf.image.resize_images(h_r,[256,256])
      h_gxl = tf.image.resize_images(h_g,[256,256])
      h_bxl = tf.image.resize_images(h_b,[256,256])
