...
class DCGAN(object):
  def __init__(self, sess, input_height=64, input_width=64, input_water_height=64, input_water_width=64, is_crop=False,
    ...
    self.gf_dim = gf_dim
    self.df_dim = df_dim
    self.sample_outw = 256
    self.sample_outh = 256
  ...
  def build_model(self):
    ...
    sample_dims = [self.sample_outh,self.sample_outw, self.c_dim]
    ...
    self.sample_depth_inputs = tf.placeholder(
      tf.float32, [self.batch_size] + [self.sample_outh,self.sample_outw,1], name='depth')
...
#print(self.sess.run('wc_generator/g_vig/w:0'))
       ...
       # if (np.mod(epoch,20)==1) and (checkprint==0):
        #print(self.D)
        #print(self.D_)
        if (epoch == self.save_epoch) and (checkprint == 0):
            ...
            for idx in xrange(0, sample_batch_idxs):
                ...
                if self.is_crop:
                    sample_air_batch = [scipy.misc.imresize(scipy.misc.imread(sample_air_batch_file),(self.sample_outh,self.sample_outw,3)) for sample_air_batch_file in sample_air_batch_files]
                    sample_water_batch = [scipy.misc.imresize(scipy.misc.imread(sample_water_batch_file),(self.sample_outh,self.sample_outw,3)) for sample_water_batch_file in sample_water_batch_files]
...
#+tf.mul(B,(1.0-eta_d))
    ...
    #  print(self.h2z)
     ...
     # h_out = tf.mul(h2z, h0)
      ...
      h2z = self.h2z
      h_rxl = tf.image.resize_images(h_r,[self.sample_outh,self.sample_outw],method=2)
      h_gxl = tf.image.resize_images(h_g,[self.sample_outh,self.sample_outw],method=2)
      h_bxl = tf.image.resize_images(h_b,[self.sample_outh,self.sample_outw],method=2)
  #    h_rxl = tf.image.resize_images(h_rxlt,[256,256],method=3)
  #    h_gxl = tf.image.resize_images(h_gxlt,[256,256],method=3)
  #    h_bxl = tf.image.resize_images(h_bxlt,[256,256],method=3)
     ...
     # Augment 4: vignetting model
      ...
      with tf.variable_scope("g_vig",reuse=True):
          A = tf.get_variable('g_amp', [1],
              initializer=tf.truncated_normal_initializer(mean=0.7,stddev=0.3))
      h1 = tf.mul(A,kernel)
      h1_xl = tf.image.resize_images(h1,[self.sample_outh,self.sample_outw],method=2)
      #h1_xl = tf.image.resize_images(h1_xlt,[256,256],method=3)
...
#      h1 = h0 + tf.mul(beta,(1.0-tf.exp(eta_d)))
  ...
  def read_depth_sample(self, filename):
    ...
    depth=depth_mat["depth"]
    if self.is_crop:
      depth = scipy.misc.imresize(depth,(self.sample_outh,self.sample_outw),mode='F')
    depth = np.array(depth).astype(np.float32)
    depth = np.multiply(self.max_depth,np.divide(depth,depth.max()))
