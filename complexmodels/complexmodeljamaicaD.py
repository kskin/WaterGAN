...
#print(self.sess.run('wc_generator/g_vig/w:0'))
        ...
        #print(self.D_)
            ...
            checkprint = 1
            sample_batch_idxs = 5000// config.batch_size
            ...
            while(num_D < 5000):
                ...
                #sample_waterdepth_batch_files = waterdepth_data[idx*100:(idx+1)*100]
                    if self.is_crop:
                        sample_air_batch = [scipy.misc.imresize(scipy.misc.imread(sample_air_batch_file),(256,256,3)) for sample_air_batch_file in sample_air_batch_files]
                        sample_water_batch = [scipy.misc.imresize(scipy.misc.imread(sample_water_batch_file),(256,256,3)) for sample_water_batch_file in sample_water_batch_files]
                ...
                #sample_depth_batch_images = np.expand_dims(sample_depth_batch,axis=3)
                   ...
                   # num_D = 0
                    ...
                    sample_fake_images = sample_ims[:,0:256,0:256,0:3]
                    sample_fake_images_small = np.empty([0,64,64,3])
                    for img_idx in range(0,self.batch_size):
                        #print("here")
                     #   out_name = "sample_finaljamaica/fake_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                     ...
                     #   sample_im = np.squeeze(sample_im)
                      ...
                      #  out_name = "sample_finaljamaica/air_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                      #  sample_im = sample_air_images[img_idx,0:256,0:256,0:3]
                        #sample_im = conv_ims[img_idx,0:256,0:256,0:3]
                      #  sample_im = np.squeeze(sample_im)
                      #  scipy.misc.imsave(out_name,sample_im)
                      #  out_name = "sample_finaljamaica/depth_%0d_%02d_%02d.mat" % (epoch, img_idx,idx)
                       # sample_im = sample_depth_images[img_idx,0:256,0:256,0]
                       # sample_im = np.squeeze(sample_im)
                       # sio.savemat(out_name,{'depth':sample_im})
                        sample_fake = sample_fake_images[img_idx,0:256,0:256,0:3]
                ...
                #print(sample_fake_images_small.shape)
                    ...
                    print(disc_result)
                #outfile = "my_results-jamaica.txt"
                #fh = open(outfile,"a")
                    for img_idx in range(0,self.batch_size):
                        res = int(disc_result[img_idx])
                        #print res
                        if (res == 1):
                            num_D = num_D + 1
                            print(num_D)
                            #out_name = "result_%0d_%02d_%02d.png" % (epoch, img_idx,idx)
                            out_name = "sample_finaljamaicaD/FO_%05d.png" % (num_D)
                            sample_im = sample_fake_images[img_idx,0:256,0:256,0:3]
                            sample_im = np.squeeze(sample_im)
                            #print(sample_im.shape)
                            scipy.misc.imsave(out_name,sample_im)
                            out_name = "sample_finaljamaicaD/A_%05d.png" % (num_D)
                            sample_im = sample_air_images[img_idx,0:256,0:256,0:3]
                            #sample_im = conv_ims[img_idx,0:256,0:256,0:3]
                            sample_im = np.squeeze(sample_im)
                            scipy.misc.imsave(out_name,sample_im)
                            out_name = "sample_finaljamaicaD/D_%05d.mat" % (num_D)
                            sample_im = sample_depth_images[img_idx,0:256,0:256,0]
                            sample_im = np.squeeze(sample_im)
                            sio.savemat(out_name,{'depth':sample_im})
                            sample_fake = sample_fake_images[img_idx,0:256,0:256,0:3]
