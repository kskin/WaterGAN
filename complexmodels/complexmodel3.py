...
# +0.01*tf.nn.l2_loss(eta_b)+ 0.01*tf.nn.l2_loss(eta_g)+ 0.01*tf.nn.l2_loss(eta_r)

    # constrain eta > 0
    self.eta_r_loss = -tf.minimum(tf.reduce_min(eta_r),0)
    self.eta_g_loss = -tf.minimum(tf.reduce_min(eta_g),0)
    self.eta_b_loss = -tf.minimum(tf.reduce_min(eta_b),0)
 #   self.beta_loss = -tf.minimum(tf.reduce_min(beta),0)
    self.g_loss = self.g_loss 
 #+ self.eta_r_loss + self.eta_g_loss +self.eta_b_loss # + self.beta_loss
   ...
   # self.D_res = tf.summary.scalar("D_realdata", self.D)
    #self.D__res = tf.summary.scalar("D_fakedata", self.D_)
...
#print(self.sess.run('wc_generator/g_vig/w:0'))
  ...
  def wc_generator(self, z, image, depth, y=None):
    with tf.variable_scope("wc_generator") as scope:
      ...
      h0 = tf.mul(image,eta_d)+tf.mul(B,(1.0-eta_d))
     ...
     ########################################################
      ...
      h_out = tf.mul(h1,h2)
      return h0, eta_r,eta_g,eta_b, beta
