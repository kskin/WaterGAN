...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    self.G,eta_r,eta_g,eta_b = self.wc_generator(self.z,air_inputs, depth_inputs)
    if self.is_stereo:
    ...
    lambda_r = 100
    lambda_b = 100
    self.eta_r_loss = -tf.minimum(tf.reduce_min(eta_r),0)*lambda_r
    self.eta_g_loss = -tf.minimum(tf.reduce_min(eta_g),0)*lambda_g
    self.eta_b_loss = -tf.minimum(tf.reduce_min(eta_b),0)*lambda_b
...
#+tf.mul(B,(1.0-eta_d))
     ...
     ########################################################
      ...
      h_out = tf.mul(h1,h2)
      h_outv = tf.mul(h0,h1)
      return h_out, eta_r,eta_g,eta_b
